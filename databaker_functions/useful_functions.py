def loadxlstabs(inputfile, sheetids="*", verbose=True):
    if verbose:
        print(
            "Loading %s which has size %d bytes"
            % (inputfile, os.path.getsize(inputfile))
        )
    ...
    if sheetids != "*":
        ...
        assert len(sheetids) == len(tabnames), (
            "Number of selected tables disagree",
            "len(sheetids) == len(tabnames)",
            len(sheetids),
            len(tabnames),
        )
...
class ConversionSegment:
    ...
    def __init__(
        ...
        for dimension in self.dimensions:
            ...
            assert dimension.hbagset is None or dimension.hbagset.table is tab, (
                "dimension %s from different tab" % dimension.name
            )

        self.numheaderadditionals = sum(
            1
            for dimension in self.dimensions
        ...
        if isinstance(self.segment, xypath.xypath.Bag):
            ...
            self.obslist = list(
                self.segment.unordered_cells
            )  # list(segment) otherwise gives bags of one element
            self.obslist.sort(key=lambda cell: (cell.y, cell.x))
        else:
            assert isinstance(
                self.segment, (tuple, list)
            ), "segment needs to be a Bag or a list, not a %s" % type(self.segment)
            ...
            if dimension.hbagset is not None:  # filter out TempValue headers
                for hcell in dimension.hbagset.unordered_cells:
                    ...
                    if hcell in dimension.cellvalueoverride:
                        ...
                        val = str(
                            dimension.cellvalueoverride[type(hcell.value)](hcell.value)
                        )
                    else:
                        val = sval
                    if val != sval:
    ...
    def lookupobs(self, ob):
        ...
        if not isinstance(ob.value, float):
            if ob.properties[
                "richtext"
            ]:  # should this case be implemented into the svalue() function?
                sval = richxlrd.RichCell(
                    ob.properties.cell.sheet, ob.y, ob.x
                ).fragments.not_script.value
            else:
                sval = svalue(ob)

            if template.SH_Split_OBS:
                assert template.SH_Split_OBS == databaker.constants.DATAMARKER, (
                    template.SH_Split_OBS,
                    databaker.constants.DATAMARKER,
                )
                ob_value, dm_value = re.match(
                    r"([-+]?[0-9]+\.?[0-9]*)?(.*)", sval
                ).groups()
                dval = {}
                if dm_value:
    ...
    def guesstimeunit(self):
        ...
        ctu = collections.Counter(
            dval[template.TIMEUNIT] for dval in self.processedrows
        )
        if len(ctu) == 1:
            return "TIMEUNIT='%s'" % list(ctu.keys())[0]
        return "multiple TIMEUNITs: %s" % ", ".join(
            "'%s'(%d)" % (k, v) for k, v in ctu.items()
        )

    def fixtimefromtimeunit(
        self,
    ):  # this works individually and not across the whole segment homogeneously
        for dval in self.processedrows:
            dval[template.TIME] = Ldatetimeunitforce(
                dval[template.TIME], dval[template.TIMEUNIT]
            )
    ...
    def process(self):
        ...
        if self.processtimeunit:
            if template.SH_Create_ONS_time and (
                (template.TIMEUNIT not in kdim) and (template.TIME in kdim)
            ):
...
class HDim:
    ...
    def __init__(
        self, hbagset, label, strict=None, direction=None, cellvalueoverride=None
    ):
        self.label = label
        self.name = label

        self.cellvalueoverride = (
            cellvalueoverride or {}
        )  # do not put {} into default value otherwise there is only one static one for everything
        assert not isinstance(
            hbagset, str
        ), "Use empty set and default value for single value dimension"
        ...
        if self.hbagset is None:  # single value type
            ...
            assert (
                len(cellvalueoverride) == 1 and None in cellvalueoverride
            ), "single value type should have cellvalueoverride={None:defaultvalue}"
            return

        assert isinstance(
            self.hbagset, xypath.xypath.Bag
        ), "dimension should be made from xypath.Bag type, not %s" % type(self.hbagset)
        self.strict = strict
    ...
    def celllookup(self, scell):
        ...
        def same_row_col(a, b):
            return (a.x - b.x == 0 and self.direction[0] == 0) or (
                a.y - b.y == 0 and self.direction[1] == 0
            )
        ...
        if second_best_cell and mult(best_cell) == mult(second_best_cell):
            raise xypath.LookupConfusionError(
                "{!r} is as good as {!r} for {!r}".format(
                    best_cell, second_best_cell, scell
                )
            )
    ...
    def headcellval(self, hcell):
        ...
        if hcell is not None:
            assert isinstance(
                hcell, xypath.xypath._XYCell
            ), "celllookups should only go to an _XYCell"
            if hcell in self.cellvalueoverride:
                val = self.cellvalueoverride[hcell]
                assert isinstance(val, (str, float, int)), (
                    "Override from hcell value should go directly to a str,float,int,None-value (%s)"
                    % type(val)
                )
                return val
            val = svalue(hcell)
        ...
        if val in self.cellvalueoverride:
            ...
            assert val is None or isinstance(
                val, (str, float, int)
            ), "Override from value should only be str,float,int,None (%s)" % type(val)
    ...
    def cellvalobs(self, ob):
        ...
        if isinstance(ob, xypath.xypath.Bag):
            ...
            ob = ob._cell
        assert isinstance(
            ob, xypath.xypath._XYCell
        ), "Lookups only allowed on an obs cell"
        ...
        if ob in self.cellvalueoverride:
            val = self.cellvalueoverride[
                ob
            ]  # knock out an individual obs for this cell
            assert isinstance(
                val, str
            ), "Override from obs should go directly to a string-value"
            return None, val
    ...
    def AddCellValueOverride(self, overridecell, overridevalue):
        ...
        assert isinstance(
            overridecell, xypath.xypath._XYCell
        ), "Lookups only allowed on an obs cell"
        ...
        assert overridevalue is None or isinstance(
            overridevalue, (str, float, int)
    ...
    def valueslist(self):
        ...
        return [
            self.headcellval(cell)
            for cell in sorted(
                self.hbagset.unordered_cells, key=lambda cell: (cell.y, cell.x)
            )
        ]
    ...
    def checkvalues(self, vlist):
        ...
        for cell, v in zip(scells, vlist):
            ...
            if nv != v:
                warnings.warn(
                    "checkvalues mismatch in cell (%d,%d) cell value '%s' doesn't match '%s'"
                    % (cell.x, cell.y, nv, v)
                )
                return False
        return True
